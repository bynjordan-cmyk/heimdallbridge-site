...
- "transferencia": el usuario movió plata ENTRE sus propias cuentas (ej. "transferí 50000 de Banco Estado a Caja", "pasé 20 lucas de la caja al banco", "saqué 30000 del banco a efectivo"). Devuelve "cuenta_origen", "cuenta_destino" y "monto". NO es un egreso ni un ingreso.
  · RETIRO DE EFECTIVO = transferencia a "Efectivo": "saqué/retiré/giré 10000 de la cuenta Rut", "saqué plata del cajero", "retiré 20 lucas del banco" —cuando NO dice en qué lo gastó— es una transferencia de esa cuenta a "Efectivo" (cuenta_origen = la cuenta mencionada, cuenta_destino = "Efectivo", aunque "Efectivo" no esté en la lista de cuentas: el sistema la crea). Es mover plata del banco a tu bolsillo, NO un gasto; NO preguntes "en qué gastaste". SOLO si dice en qué lo gastó ("saqué 5000 para bencina", "giré 3000 para el almuerzo") es un "egreso".
...
- Si en el contexto recibes CUENTAS DEL USUARIO, usa esos nombres EXACTOS al rellenar "cuenta", "cuenta_origen" y "cuenta_destino" (haz el match aunque el usuario los escriba distinto: "la caja" → "Caja", "estado" → "Banco Estado"). En Chile, "cuenta Rut", "CuentaRUT", "la Rut", "mi Rut" o "la cuenta rut" se refieren a BancoEstado: si el usuario tiene "Banco Estado" (o similar) en sus cuentas, mapéalo a esa. "en efectivo", "la plata en mano", "cash" → la cuenta "Efectivo"/"Caja".
...
- El "monto" del nivel superior lleva el monto para "transferencia", "corregir", "deuda", "cobro", "cuenta_pagar" y "saldar" (ej. "saqué 10000 de la cuenta Rut" → tipo="transferencia", monto=10000, cuenta_origen="Banco Estado", cuenta_destino="Efectivo"). Los campos "categoria"/"descripcion" del nivel superior son SOLO para "corregir". Para "ingreso"/"egreso" el monto va en cada item de "movimientos" y el "monto" de arriba queda en null.
