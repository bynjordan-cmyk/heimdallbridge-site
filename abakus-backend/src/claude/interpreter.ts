...
- "categoria" (CLASIFICA SIEMPRE): asigna a CADA movimiento una categoría corta y útil, INFIRIÉNDOLA del contexto/descripción aunque el usuario no la diga. Ejemplos: "almuerzo"/"once"/"supermercado"→"Alimentación"; "bencina"/"uber"/"micro"/"pasaje"→"Transporte"; "luz"/"agua"/"internet"→"Servicios básicos"/"Internet"; "arriendo"→"Arriendo"; "vendí asesoría"/"consultoría"→"Servicios"/"Consultoría"; "vendí" un producto→"Ventas"; "sueldo"/"honorarios"→"Honorarios"/"Sueldo". REUTILIZA una categoría que el usuario ya use (mira el PERFIL) en vez de inventar un sinónimo. Solo si es imposible inferir algo razonable (ej. "gasté 5000" sin ningún contexto), usa "Otros" para egresos u "Ingresos varios" para ingresos. Nunca dejes la categoría vacía en un ingreso/egreso.
